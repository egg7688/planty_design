...
DBpia 사이트에서 직접 로그인한 뒤 앱으로 돌아와 키워드를 입력하면 DBpia와 Google Scholar 호환 API에서 학술 검색 결과를 수집하고, 요약 보고서를 생성한 뒤 이메일로 발송하는 웹사이트입니다.

## 기능

- DBpia 로그인 페이지 이동 및 로그인 완료 확인 흐름
- 키워드와 이메일 입력 폼
...
- `PREMIUM_ACCESS_CODE`: 선택값입니다. DBpia 로그인 완료 확인 대신 별도 접근 코드 방식도 유지할 때 사용합니다.
...
이 프로젝트는 사용자를 `https://www.dbpia.co.kr/member/login`으로 이동시켜 DBpia 사이트에서 직접 로그인하게 합니다. 앱은 DBpia 계정 비밀번호를 받거나 저장하지 않고, 브라우저의 DBpia 세션 쿠키에도 접근하지 않습니다.
