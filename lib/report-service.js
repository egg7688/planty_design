...
async function createPremiumSession({ email, accessCode }) {
  const normalizedEmail = String(email || "").trim().toLowerCase();
  const normalizedCode = String(accessCode || "").trim();
  const expectedCode = getPremiumAccessCode();

  if (!isEmail(normalizedEmail)) {
    throw httpError(400, "유료 로그인에 사용할 이메일 형식이 올바르지 않습니다.");
  }
  ...
  const token = signSession({ email: normalizedEmail, plan: "premium", expiresAt });
